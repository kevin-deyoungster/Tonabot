...
def search_page(term, page_no):
    ...
    for content in ads:
        ...
        if(item):
            descr, condition, number, vendor, size, date = inspect_iphone(
                tonaton + item['url'])
            extras = {'description': descr, 'condition': condition,
                      'number': number, 'dealer': vendor, 'size': str(size), 'date': date}
            item = {**item, **extras}
            del item['url']
            del item['description']
            # print(item['title'] + ' - ' + item['price'] + ' - ' + item['condition'] + ' - ' + str(item['dealer']) + ' - ' + item["number"])
            ads_dict.append(item)
            show_notifications("Tonabot!", item["price"] + "\n" + descr, 10)
    return ads_dict
